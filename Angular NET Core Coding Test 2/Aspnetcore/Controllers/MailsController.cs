...
namespace WebApi.Controllers
{
    ...
    public class MailsController : ControllerBase, IDisposable
    {
        ...
        {
            _log.LogDebug(
                "GetSentFolder() starts - {@paramFolderId}, {@pageNumber}, {@rowsOfPage}", 
                paramFolderId, pageNumber, rowsOfPage);

            HttpContext.Response.RegisterForDispose(_disposable);
            int userId = UserService.GetUserIdFromToken(Request.Headers["Authorization"], _appSettings.Secret);
            int totalRowsCount = 0;
            IEnumerable<MailModel> paginatedMailResults = new List<MailModel>();

            // Try validate param 
            bool isValid = TryValidateGetSentFolder(pageNumber, rowsOfPage, out string errorMessage);
            ...
            IEnumerable<MailModel> sentMailsResult = _mailService.GetSentMailsByFolder(paramFolderId, userId);

            if (sentMailsResult != null)
            {
                int offSetValue = rowsOfPage * (pageNumber - 1);
                totalRowsCount = sentMailsResult.Count();
                paginatedMailResults = sentMailsResult
                                        .Skip(offSetValue)
                                        .Take(rowsOfPage);
            }

            return Ok(new MailsPageObj
            {
                results = paginatedMailResults,
        ...
        #region Validation
        private bool TryValidateGetSentFolder(int pageNumber, int rowsOfPage, out string errorMessage)
        {
            errorMessage = string.Empty;

            if (pageNumber < 1)
            {
                errorMessage = ErrorMessageConstants.INVALID_PAGE_NUMBER;
                return false;
            }

            if (rowsOfPage < 0)
            {
                errorMessage = ErrorMessageConstants.INVALID_ROWS_OF_PAGE;
                return false;
            }
